...
class MLP(nn.Module):
    ...
    def __init__(self, features_in, layer_sizes, nonlinear_act=nn.LeakyReLU, act_last_layer=True, bias=True):
        super().__init__()
        layers = nn.ModuleList()

        for i, hidden in enumerate(layer_sizes):
            layers.append(nn.Linear(features_in, hidden, bias=bias))
